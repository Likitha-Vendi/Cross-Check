...
public class AuthService {
    private static final Set<String> PUBLIC_REGISTRATION_ROLES = Set.of("HR", "RECRUITER");
    ...
    public Map<String, Object> register(String name, String email, String password, String role) {
        ...
        if (!PUBLIC_REGISTRATION_ROLES.contains(cleanRole))
            throw new IllegalArgumentException("Only HR and Recruiter accounts can be registered.");
        if (repo.findByEmailIgnoreCase(cleanEmail).isPresent())
    ...
    public User createStaffAccount(String name, String email, String password, String role) {
        String cleanRole = role == null ? "" : role.trim().toUpperCase(Locale.ROOT);
        if (!PUBLIC_REGISTRATION_ROLES.contains(cleanRole))
            throw new IllegalArgumentException("Admin can create only HR or Recruiter accounts.");
        String cleanEmail = email == null ? "" : email.trim().toLowerCase(Locale.ROOT);
        if (repo.findByEmailIgnoreCase(cleanEmail).isPresent())
            throw new IllegalStateException("An account with this email already exists.");
        if (name == null || name.trim().length() < 2) throw new IllegalArgumentException("Please enter a valid full name.");
    ...
    public User require(String auth) {
        ...
        if (u == null || !u.isActive()) throw new RuntimeException("Unauthorized");
        return u;
    }
